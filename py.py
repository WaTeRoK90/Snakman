...
fon_menu = pygame.image.load('wall.jpg').convert()
fon_menu = pygame.transform.scale(img,(HEIGHT, WIDTH))
...
# first game cycle(for resets)
while True:

    # Проверка переменной рестарт
    if reset == 1:
        reset = 0
    ...
    while True:
        ...
        if status =="game":
            ...
            # game over 
            if x < 0 or x > HEIGHT - SIZE or y < 0 or y > WIDTH - SIZE or len(snake) != len(set(snake)):
                while True:
